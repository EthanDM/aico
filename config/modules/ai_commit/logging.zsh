...
function log_api_response() {
    ...
    # Always show API responses in debug mode or if specifically enabled
    if [[ "$debug" == "true" || "$log_responses" == "true" ]]; then
        log_debug "API Response Details:"
        
        # Send colored output to stderr
        echo "${LOG_COLORS[DEBUG]}" >&2
        
        # Try to pretty print JSON, fallback to raw if not valid JSON
        if echo "$response" | jq '.' >/dev/null 2>&1; then
            # Valid JSON - pretty print with specific fields masked
            echo "$response" | jq '
                if .choices then
                    .choices[].message.content |= sub("```.*```"; "[CONTENT TRUNCATED]"; "m")
                else
                    .
                end
            ' 2>/dev/null >&2
